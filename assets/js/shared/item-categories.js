(() => {
    ...
    async function load({ force = false, token = "" } = {}) {
        ...
        const payload = await window.CriptaApp.api.get("api/item-categories", {
            cache: false,
    ...
    async function save(categories, expectedRevision, { token = "" } = {}) {
        const authToken = String(token || window.CriptaDiscordAuth?.getToken?.() || "").trim();
        if (!authToken) throw new Error("Accedi come DM per modificare le categorie degli oggetti.");
        const normalized = (Array.isArray(categories) ? categories : []).map(normalizeCategory).filter(Boolean);
        const payload = await window.CriptaApp.api.post("api/item-categories", {
        ...
        window.CriptaApp.api.clearCache?.("api/data/items");
        window.CriptaApp.api.clearCache?.("api/item-categories");
        return structuredClone(registry);
    }
    ...
    window.CriptaItemCategories = { clear, load, normalizeCategory, normalizeId, normalizeRegistry, resolve, save };
})();
