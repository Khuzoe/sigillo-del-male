...
function versionedBestiaryDetailModuleUrl(path, baseUrl) {
    ...
    url.searchParams.set("v", BESTIARY_DETAIL_MODULE_VERSION);
    return url.toString();
}
...
window.CriptaApp.onPageReady("creature", async () => {
    ...
    let createMode = params.get("new") === "1";
    ...
    let foundryExportModulePromise = null;
    ...
    function bindCreatureMediaDropTargets() {
        bindCreatureMediaDropTarget('[data-action="upload-image"]', {
            property: "image",
            folder: "creatures/bestiary",
            suffix: "",
            label: "immagine creatura"
        });
        bindCreatureMediaDropTarget('[data-action="upload-token-image"]', {
            property: "tokenImage",
    ...
    function bindCreatureMediaDropTarget(selector, uploadOptions) {
        const target = root.querySelector(selector);
        if (!target) return;
        const clearDragState = () => target.classList.remove("is-drag-over");
        target.addEventListener("dragenter", (event) => {
            if (!hasImageDragPayload(event)) return;
            event.preventDefault();
            event.stopPropagation();
        ...
        target.addEventListener("drop", async (event) => {
            event.preventDefault();
            event.stopPropagation();
            const file = getDataTransferImageFile(event);
            if (!file) {
                clearDragState();
                return;
            }
            clearDragState();
            await uploadCreatureMedia({ ...uploadOptions, file });
        });
    }

    function hasImageDragPayload(event) {
        const types = Array.from(event.dataTransfer?.types || []);
        if (types.includes("Files")) return true;
        return getDataTransferImageFile(event) !== null;
    }

    function getDataTransferImageFile(event) {
        const files = Array.from(event.dataTransfer?.files || []);
        return files.find((file) => {
            const type = String(file?.type || "").toLowerCase();
            const name = String(file?.name || "").toLowerCase();
            return type.startsWith("image/") || /\.(avif|gif|jpe?g|png|webp)$/.test(name);
        }) || null;
    }
    ...
    function loadFoundryExportModule() {
        if (window.CriptaBestiaryFoundryExport) return Promise.resolve(window.CriptaBestiaryFoundryExport);
        if (foundryExportModulePromise) return foundryExportModulePromise;

        foundryExportModulePromise = new Promise((resolve, reject) => {
            const existingScript = document.querySelector("script[data-bestiary-foundry-export-module]");
            if (existingScript) {
                existingScript.addEventListener("load", () => resolve(window.CriptaBestiaryFoundryExport), { once: true });
                existingScript.addEventListener("error", () => reject(new Error("Caricamento export Foundry fallito.")), { once: true });
                return;
            }

            const mainScript = document.querySelector('script[src*="assets/js/pages/bestiary-detail.js"]');
            const baseUrl = mainScript?.src || new URL("../../assets/js/pages/bestiary-detail.js", window.location.href).toString();
            const script = document.createElement("script");
            script.src = versionedBestiaryDetailModuleUrl("../shared/bestiary-foundry-export.js", baseUrl);
            script.defer = true;
            script.dataset.bestiaryFoundryExportModule = "true";
            script.addEventListener("load", () => {
                if (window.CriptaBestiaryFoundryExport) {
                    resolve(window.CriptaBestiaryFoundryExport);
                    return;
                }
                reject(new Error("Modulo export Foundry non inizializzato."));
            }, { once: true });
            script.addEventListener("error", () => reject(new Error("Caricamento export Foundry fallito.")), { once: true });
            document.head.appendChild(script);
        });

        return foundryExportModulePromise;
    }

    function getFoundryExportRuntimeContext() {
        return {
            ABILITY_DAMAGE_TYPE_OPTIONS,
            CONDITION_IMMUNITY_OPTIONS,
            ensureFoundryMonsterData,
            getMonsterAbilities,
            resolveImageUrl,
            normalizeFoundryAbilities,
            toLeadingNumberOrNull,
            toNumberOrNull,
            normalizeFoundryMovement,
            normalizeFoundrySenses,
            mapWikiTypeToFoundry,
            toNumberOrString,
            parseCsvLower,
            normalizeDamageTypes,
            buildFoundryResource,
            normalizeFoundrySkills,
            tokenSizeForFoundrySize,
            getAbilityRider,
            getAbilityDamageParts,
            getAbilityRiderDamageParts,
            foundryIconFromFa,
            inferAbilityKind,
            activationFromSection,
            parseRangeValue,
            parseRangeUnits,
            buildFoundryItemUses,
            normalizeConditionImmunities,
            getAbilityRechargeValue,
            inferActionType,
            isAttackAbility,
            isRangedAttackAbility,
            getPrimaryAbilityDamageType,
            isPhysicalAbilityDamageType,
            getFoundryProficiency,
            slugify,
            escapeHtml,
            rechargeLabel,
            conditionLabel,
    ...
    async function downloadFoundryActor() {
        getMonsterAbilities(state.creature).forEach(applyAbilityDefaultsForKind);
        const exportModule = await loadFoundryExportModule();
        const actor = exportModule.buildActorExport(state.creature, getFoundryExportRuntimeContext());
    ...
    async function saveEdit() {
        ...
        try {
            ...
            state.creature = cleanCreature || pruneCreature(structuredCloneSafe(sourceCreature));
            state.version = saved.payload?.version ?? state.version;
            state.source = "kv";
            state.editing = false;
            state.dirty = false;
            editButton?.classList.remove("is-editing");
            if (createMode || creatureId !== state.creature.id) {
                const nextUrl = new URL(window.location.href);
                nextUrl.searchParams.delete("new");
                nextUrl.searchParams.set("id", state.creature.id);
    ...
    async function persistCreatureMediaReference(property, value) {
        ...
        const saved = await saveBestiaryDocumentWithRetry(token, (records) => {
            ...
            savedEntry = baseEntry;
            if (index >= 0) nextData[index] = baseEntry;
            else nextData.push(baseEntry);
            repairPlaceholderCreatureIds(nextData);
            return nextData;
        });
        if (savedEntry?.id) state.creature.id = savedEntry.id;
        state.version = saved.payload?.version ?? state.version;
        state.source = "kv";
    }
...
async function loadBestiaryDocument() {
    try {
        if (typeof window.CriptaApp?.api?.get === "function") {
            const payload = await window.CriptaApp.api.get("api/data/bestiary", { cache: false, query: { _: Date.now() } });
...
async function saveBestiaryDocumentWithRetry(token, buildData, attempts = 5) {
    let lastPayload = null;
    let lastStatus = 0;
    for (let attempt = 0; attempt < attempts; attempt += 1) {
        clearBestiaryApiCache();
        const loaded = await loadBestiaryDocument();
        ...
        if (response.status === 409 || payload?.code === "VERSION_CONFLICT") {
            clearBestiaryApiCache();
            await delayBestiaryRetry(220 + attempt * 320);
            continue;
        }
        if (!response.ok || payload?.ok === false) throw new Error(payload?.error || `HTTP ${response.status}`);
        clearBestiaryApiCache();
...
function clearBestiaryApiCache() {
    window.CriptaApp?.api?.clearCache?.("api/data/bestiary");
}

function delayBestiaryRetry(ms) {
...
function isDefaultNewCreatureName(value) {
    return slugify(value || "") === "nuovo-mostro";
}

function isNewCreaturePlaceholderId(value) {
...
function makeUniqueCreatureId(baseId, records, existingIndex = -1) {
    const base = slugify(baseId || "creatura") || "creatura";
    const used = new Set((Array.isArray(records) ? records : [])
        .map((entry, index) => index === existingIndex ? "" : slugify(entry?.id || entry?.name || ""))
        .filter(Boolean));
