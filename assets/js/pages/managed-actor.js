(() => {
    ...
    window.CriptaApp.onPageReady("managed-actor", async () => {
        ...
        try {
            ...
            const canEdit = currentDocument?.permissions ? currentDocument.permissions.canEdit === true : Boolean(token);
            renderManagedActor(root, currentDocument, canEdit);
        } catch (error) {
    ...
    function renderManagedActor(root, actor, canEdit) {
        ...
        root.innerHTML = `
            ...
            <section class="managed-actor-hero">
                <div class="managed-actor-art">
                    ...
                    <div class="managed-actor-avatar">${renderImage(avatarPath, actor.name || "Actor")}</div>
                    ${tokenPath ? `<figure class="managed-actor-token"><img src="${escapeAttr(resolveMedia(tokenPath))}" alt="Token di ${escapeAttr(actor.name || "Actor")}"><figcaption>Token mappa</figcaption></figure>` : ""}
                </div>
                <div class="managed-actor-title">
                    <div class="managed-actor-kicker"><span></span> ${escapeHtml(actor.actorType || "actor")} gestito</div>
                    <h1>${canEdit ? renderManagedActorControl("name", "text", actor.name || "Actor", { className: "managed-actor-name-input" }) : escapeHtml(actor.name || "Actor")}</h1>
                    ...
                    <p class="managed-actor-intro">Statistiche, equipaggiamento e immagini restano collegate all'Actor originale. Le varianti token possono essere curate indifferentemente qui o nel Tokenizer.</p>
                </div>
            </section>
            ${canEdit ? `<div class="managed-save-toolbar"><div class="managed-save-toolbar-copy"><i class="fas fa-pen-ruler" aria-hidden="true"></i><span><strong>Modalità modifica</strong><small>Statistiche, tratti, immagini e varianti vengono salvati insieme.</small></span></div><span class="managed-save-status" data-managed-status></span><button type="button" class="button-gold-outline managed-primary-action" data-managed-save><i class="fas fa-cloud-arrow-up"></i> Salva modifiche</button></div>` : ""}
            ${renderSectionNavigation({ abilities, skills, traits, variants, effects, attackEntries, spellEntries, inventoryEntries, canEdit })}
            <div class="managed-actor-panels">
                ${renderCoreStats(attributes, details, actor.runtime || {}, actor.actorType, traits, definition.spellSlots || {}, canEdit)}
                ${renderAbilities(abilities, canEdit)}
                ${renderSkills(skills, canEdit)}
                ${renderTraits(traits, canEdit)}
                ${canEdit ? renderAdmin(actor) : ""}
                ${variants.length || canEdit ? renderManagedVariantsEditor(variants, canEdit) : ""}
                ${effects.length || canEdit ? renderManagedEffects(effects, canEdit) : ""}
                ${renderEntries("Attacchi e capacità", attackEntries, canEdit, "managed-capabilities")}
                ${renderEntries("Incantesimi", spellEntries, canEdit, "managed-spells")}
                ${renderEntries("Inventario", inventoryEntries, canEdit, "managed-inventory")}
            </div>
        `;
    ...
    function setupManagedImagePreviews(root) {
        ...
        root.querySelectorAll("[data-managed-file]").forEach((input) => input.addEventListener("change", () => {
            ...
            if (["avatar", "token"].includes(slot)) {
                ...
                const heroImage = root.querySelector(slot === "avatar" ? ".managed-actor-avatar img" : ".managed-actor-token img");
                if (heroImage) heroImage.src = previewUrl;
                return;
            }
    ...
    function renderSectionNavigation({ abilities, skills, traits, variants, effects, attackEntries, spellEntries, inventoryEntries, canEdit }) {
        const links = [
            ["managed-stats", "fa-chart-simple", "Statistiche", true],
            ...
            ["managed-traits", "fa-fingerprint", "Tratti", Object.keys(traits || {}).length > 0],
            ["managed-appearance", "fa-images", "Aspetto", canEdit],
            ["managed-variants", "fa-layer-group", "Varianti", variants.length > 0 || canEdit],
            ["managed-effects", "fa-wand-magic-sparkles", "Effetti", effects.length > 0 || canEdit],
            ["managed-capabilities", "fa-burst", "Capacità", attackEntries.length > 0],
            ["managed-spells", "fa-wand-sparkles", "Incantesimi", spellEntries.length > 0],
            ["managed-inventory", "fa-backpack", "Inventario", inventoryEntries.length > 0]
        ].filter(([, , , visible]) => visible);
        if (links.length < 2) return "";
        return `<nav class="managed-section-nav" aria-label="Sezioni della scheda"><span class="managed-section-nav-label">Vai a</span><div>${links.map(([id, icon, label]) => `<a href="#${id}"><i class="fas ${icon}" aria-hidden="true"></i><span>${escapeHtml(label)}</span></a>`).join("")}</div></nav>`;
    }
    ...
    function renderCoreStats(attributes, details, runtime, actorType, traits, spellSlotDefinitions = {}, canEdit = false) {
        ...
        const cards = stats.map((entry) => canEdit && entry.editable !== false
            ...
            : `<div class="managed-stat ${entry.editable === false ? "managed-stat--derived" : ""}"><i class="fas ${entry.icon}"></i><div><span>${escapeHtml(entry.label)}</span><strong>${escapeHtml(String(entry.value))}</strong>${entry.note ? `<small>${escapeHtml(entry.note)}</small>` : ""}</div></div>`).join("");
    ...
    function renderManagedSpellSlots(definitions = {}, runtime = {}, canEdit = false) {
        ...
        const slots = keys.map((key) => {
            ...
            if (!maximum && key !== "pact") return null;
    ...
    function renderManagedVariantsEditor(variants, canEdit) {
        const cards = variants.map((variant) => `<article class="managed-variant" data-managed-variant="${escapeAttr(variant.id)}">
            ...
            <div class="managed-variant-body">
                ...
                ` : `<h3>${escapeHtml(variant.name || "Variante")}</h3><p>${escapeHtml(`${variant.width || 1} × ${variant.height || variant.width || 1}`)}</p>`}
            </div>
        </article>`).join("");
    ...
    function renderEntries(title, entries, canEdit = false, sectionId = "") {
        ...
        const cards = entries.map((entry) => {
            ...
            const description = truncatePreview(stripManagedDuplicateHeading(htmlToText(entry.definition?.description || ""), entry.name), 360);
            ...
            return `<article class="managed-entry" data-managed-item-card="${escapeAttr(entry.transferId || entry.itemId || "")}">${icon ? `<img src="${escapeAttr(resolveMedia(icon))}" alt="">` : '<div class="managed-entry-icon"><i class="fas fa-dice-d20"></i></div>'}<div class="managed-entry-copy"><div class="managed-entry-title"><h3>${escapeHtml(entry.name || "Elemento")}</h3>${meta ? `<span>${escapeHtml(meta)}</span>` : ""}</div>${status}${description ? `<p>${formatManagedPreview(description)}</p>` : ""}${editor}</div></article>`;
        }).join("");
        const empty = !entries.length ? `<div class="managed-empty-state"><i class="fas ${panelIcon}"></i><strong>Nessun elemento</strong></div>` : "";
        return `<section${sectionId ? ` id="${escapeAttr(sectionId)}"` : ""} class="managed-panel managed-panel--wide managed-panel--entries"><header class="managed-panel-heading"><div><span class="managed-panel-eyebrow">Dati Foundry</span><h2><i class="fas ${panelIcon}"></i> ${escapeHtml(title)}</h2></div><span class="managed-count-badge">${entries.length}</span></header><div class="managed-entry-grid">${cards}${empty}</div>${canEdit ? renderManagedItemCreator(title) : ""}</section>`;
    }
    ...
    async function saveManagedActorPresentation(root) {
        ...
        try {
            ...
            if (Number(latest?.data?.revision || 0) !== Number(currentDocument.revision || 0)) {
                ...
                renderManagedActor(root, currentDocument, true);
    ...
    function formatDetails(details) {
        ...
        return [race, type, details.alignment].filter(Boolean).join(" · ");
    }
    ...
    function formatMovement(movement) {
        ...
        return Object.entries(movement).filter(([, value]) => Number(value) > 0).map(([key, value]) => `${key} ${value}`).join(", ");
    }

    function formatTrait(value) {
        if (!value) return "";
        if (Array.isArray(value)) return value.map((entry) => typeof entry === "object" ? entry.value || entry.name : entry).filter(Boolean).join(", ");
        if (typeof value === "object") {
            const values = Array.isArray(value.value) ? value.value : [];
            return [...values, value.custom].filter(Boolean).join(", ");
        }
        return String(value);
    }
    ...
    function formatEntryMeta(entry) {
        ...
        if (entry.type === "spell") return [`Livello ${def.level ?? 0}`, def.school].filter(Boolean).join(" · ");
        const typeLabels = { weapon: "Attacco", feat: "Capacita", equipment: "Equipaggiamento", consumable: "Consumabile", loot: "Oggetto", tool: "Strumento", container: "Contenitore" };
        return [typeLabels[entry.type] || entry.type, def.rarity, def.equipped ? "Equipaggiato" : ""].filter(Boolean).join(" · ");
    }
