(() => {
    ...
    function renderRow(category, categories, index) {
        ...
        return `<article class="npc-category-row ${category.archived ? "is-archived" : ""}" data-category-id="${escapeHtml(category.id)}" style="--category-color:${escapeHtml(category.color)}">
            ...
            <div>
                <div class="npc-category-row__usage">${category.usageCount || 0} oggetti</div>
        ...
        dialog.innerHTML = `<form class="npc-category-manager" method="dialog">
            <header class="npc-category-manager__header">
                <div class="npc-category-manager__heading">
                    ...
                    <div><small>Organizzazione della campagna</small><h2>Categorie oggetti</h2></div>
                </div>
    ...
    async function open({ onSaved } = {}) {
        ...
        let registry;
        try {
            registry = await window.CriptaItemCategories.load({ force: true });
        } catch (error) {
        ...
        dialog.addEventListener("click", async (event) => {
            ...
            if (add) {
                ...
                const id = window.CriptaItemCategories.normalizeId(name);
                if (!id) return;
            ...
            render(dialog, state);
            try {
                const saved = await window.CriptaItemCategories.save(state.categories, state.revision);
    ...
    function init({ isEditor = false, onSaved } = {}) {
        const button = document.querySelector("[data-item-category-manager]");
        if (!button) return;
    ...
    window.CriptaItemCategoryManager = { init, open };
})();
